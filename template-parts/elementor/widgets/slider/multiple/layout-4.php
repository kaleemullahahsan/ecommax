...
if ( $sliders ) {
	...
	<div id="<?php echo esc_attr($id); ?>" class="rbb-elementor-slider overflow-hidden <?php echo esc_attr($layout_type); ?> <?php echo esc_attr($layout); ?>">
		...
		<?php } ?>
		<div class="2xl:container px-[15px] mx-auto overflow-hidden">
				...
				<?php if ( $widget_title || $widget_sub_title ) { ?>
					<div class="title_block inline-block">
						<?php if ( $widget_sub_title ) { ?>
							<p class="sub-title text-[#222222] text-2xl pb-3"><?php echo esc_html($widget_sub_title); ?></p>
						<?php } ?>
						<?php if ( $widget_title ) { ?>
							<h4 class="main-title text-[#222222] text-xl"><?php echo esc_html($widget_title); ?></h4>
																					<?php
						}
						?>
					</div>
				<?php } ?>
					...
					<div class="block_slide">
						...
						foreach ( $sliders as $slider ) {
							...
							?>
							<div class="item p-2.5 elementor-repeater-item-<?php echo esc_attr($slider['_id']); ?>">
								<div class="block__content flex items-center relative duration-300 bg-white text-center rounded-[5px] px-2 md:px-5 py-5 min-h-[90px] shadow-[4px_4px_6px_1px_rgba(0,0,0,0.08)]">
									<?php if ( $slider[ $slick->get_name_setting('multiple_content_image') ]['url'] ) { ?>
										<img class="!max-w-[50px] w-[50px] h-[50px] !max-h-[50px] !inline-block" src="<?php echo esc_url($slider[ $slick->get_name_setting('multiple_content_image') ]['url']); ?>" alt="<?php echo esc_attr($slider[ $slick->get_name_setting('multiple_content_title') ]); ?>">
										<?php
									}
									?>
									<div class="pl-2 md:pl-4 text-left flex justify-between items-center w-full">
