...
<div id="<?php echo esc_attr($id); ?>" class="rbb-banner relative overflow-hidden rounded-[10px] <?php echo esc_attr($layout); ?>">
	<div class="relative h-full">
		<div class="mx-auto absolute text-left w-full z-10 bottom-[20px] left-[30px] transition-all pr-[30px]">
			<div class="block_heading">
				<div class="title_block">
					<?php if ( $title ) { ?>
						<h4 class="title text-lg font-bold text-[color:var(--rbb-general-heading-color)] uppercase mb-[7px]"><?php echo wp_kses_post($title); ?></h4>
					<?php } ?>
					<?php if ( $sub_title ) { ?>
						<p class="sub_title text-base text-[color:var(--rbb-general-body-text-color)] font-medium"><?php echo wp_kses_post($sub_title); ?></p>
					<?php } ?>
				</div>
				<?php if ( $description ) { ?>
					<div class="text-sm desc mt-2 mb-5 font-medium"><?php echo wp_kses_post($description); ?></div>
				<?php } ?>
				<?php
		...
		<div class="col-span-12 lg:col-span-12 h-full">
			<div class="<?php echo esc_attr(( $parallax ) ? 'parallax' : ''); ?> relative text-center h-full">
				...
				foreach ( $images as $image ) {
					...
					?>
					<?php if ( 0 === $img_count ) { ?>
						<div class="banner-img h-full">
							<?php
							if ( $image_link_attr_string ) {
								?>
							<a <?php echo wp_kses($image_link_attr_string, 'rbb-kses'); ?> >
							<?php } ?>
							<img alt="Image" src="<?php echo esc_url($image[ $widget->get_name_setting('image') ]['url']); ?>" class="w-full h-full duration-300 transition-all <?php echo esc_attr(( $parallax ) ? 'layer' : ''); ?> inline-block <?php echo esc_attr($css_class); ?> <?php echo esc_attr($repeater_image_class); ?>"
							<?php if ( $parallax ) { ?>
