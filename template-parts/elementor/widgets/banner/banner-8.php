...
<div id="<?php echo esc_attr($id); ?>" class="rbb-banner group relative overflow-hidden rounded-[10px] <?php echo esc_attr($layout); ?>">
	<div class="relative">
		<div class="mx-auto absolute text-left pl-10 w-full z-10 transition-all bottom-0">
			<div class="block_heading">
				...
				<?php if ( $description ) { ?>
					<div class="desc mt-3 mb-8 font-base text-sm text-[color:var(--rbb-general-heading-color)] leading-6"><?php echo wp_kses_post($description); ?></div>
				<?php } ?>
				<?php
				if ( ! empty($countdown) && ( time() < strtotime($countdown) ) ) {
					?>
					<div class="banner-countdown mb-5">
				...
				foreach ( $buttons as $button ) {
					...
					<?php if ( $button_name ) { ?>
						<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-' . esc_attr($button['_id']) . '\', event)"' : ''; ?>
							...
							class="button-banner inline-flex items-center justify-center transition-all <?php echo esc_attr($button_position); ?> <?php echo esc_attr($button_class); ?>"
							<?php
						...
						>
							<?php if ( $button_icon_status && 'before' === $button_icon_position ) { ?>
								<span class="button-icon inline-flex items-center justify-center mr-[10px] xl:w-10 xl:h-10 xl:leading-10 w-8 h-8 leading-8 rounded-full text-[#000000] bg-white hover:bg-[color:var(--rbb-general-primary-color)] hover:text-white">
										<?php
							...
							<?php if ( $button_icon_status && 'after' === $button_icon_position ) { ?>
								<span class="button-icon inline-flex items-center justify-center ml-[10px] xl:w-10 xl:h-10 xl:leading-10 w-8 h-8 leading-8 rounded-full text-[#000000] bg-white hover:bg-[color:var(--rbb-general-primary-color)] hover:text-white">
										<?php
										if ( $button_icon['value'] ) {
											Icons_Manager::render_icon($button_icon, [ 'aria-hidden' => 'true' ]);
										} else {
