...
<div id="<?php echo esc_attr($id); ?>" class="rbb-banner group relative overflow-hidden rounded-[20px] <?php echo esc_attr($layout); ?>">
	<div class="relative">
		<div class="mx-auto absolute text-center px-5 w-full z-10 bottom-[45px] transition-all duration-150 ease-in-out">
			<div class="block_heading">
				<div class="title_block">
					<?php if ( $title ) { ?>
						<h4 class="title text-lg font-bold uppercase"><?php echo wp_kses_post($title); ?></h4>
					<?php } ?>
					<?php if ( $sub_title ) { ?>
						<p class="sub_title text-sm"><?php echo wp_kses_post($sub_title); ?></p>
					<?php } ?>
				</div>
				<?php if ( $description ) { ?>
					<div class="desc mb-4 font-medium"><?php echo wp_kses_post($description); ?></div>
				<?php } ?>
				<?php
				...
				foreach ( $images as $image ) {
					...
					<?php if ( 0 === $img_count ) { ?>
						<div class="banner-img group-hover:scale-105 duration-300">
							...
							<?php } ?>
							<img alt="Image" src="<?php echo esc_url($image[ $widget->get_name_setting('image') ]['url']); ?>" class="scale-100 duration-300 group-hover:scale-105 <?php echo esc_attr(( $parallax ) ? 'layer' : ''); ?> inline-block <?php echo esc_attr($css_class); ?> <?php echo esc_attr($repeater_image_class); ?>"
							<?php if ( $parallax ) { ?>
