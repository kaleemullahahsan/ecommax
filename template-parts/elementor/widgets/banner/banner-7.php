...
<div id="<?php echo esc_attr($id); ?>" class="rbb-banner group relative overflow-hidden rounded-[10px] <?php echo esc_attr($layout); ?>">
	<div class="relative content-banner bg-white">
		<div class="mx-auto absolute text-left md:px-14 px-5 w-full z-10 top-1/2 -translate-y-1/2 transition-all content-banner-text">
			<div class="block_heading">
				<div class="title_block">
					<?php if ( $title ) { ?>
						<h4 class="title text-xs font-semibold text-[color:var(--rbb-general-primary-color)] uppercase pb-3"><?php echo wp_kses_post($title); ?></h4>
					<?php } ?>
					<?php if ( $sub_title ) { ?>
						<p class="sub_title text-4xl font-bold text-[color:var(--rbb-general-heading-color)] mb-2"><?php echo wp_kses_post($sub_title); ?></p>
					<?php } ?>
				</div>
				<?php if ( $description ) { ?>
					<div class="desc mt-5 mb-10 font-medium"><?php echo wp_kses_post($description); ?></div>
				<?php } ?>
			</div>
			<div class="flex items-center justify-start block_button">
			<?php
			if ( ! empty($countdown) && ( time() < strtotime($countdown) ) ) {
				?>
					<div class="banner-countdown mr-[17px]">
				...
				foreach ( $buttons as $button ) {
					...
					<?php if ( $button_name ) { ?>
						<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-' . esc_attr($button['_id']) . '\', event)"' : ''; ?>
							...
							class="button-banner inline-flex items-center justify-center transition-all rounded-[5px] uppercase text-xs font-bold h-[52px] px-[38px] <?php echo esc_attr($button_position); ?> <?php echo esc_attr($button_class); ?>"
							<?php
		...
		<div class="col-span-12 lg:col-span-12 content_image">
				...
				foreach ( $images as $image ) {
					...
					<?php if ( 0 === $img_count ) { ?>
						<div class="banner-img group-hover:scale-105 duration-300">
							...
							<?php } ?>
							<img alt="Image" src="<?php echo esc_url($image[ $widget->get_name_setting('image') ]['url']); ?>" class="w-full scale-100 duration-300 group-hover:scale-105 <?php echo esc_attr(( $parallax ) ? 'layer' : ''); ?> inline-block <?php echo esc_attr($css_class); ?> <?php echo esc_attr($repeater_image_class); ?>"
							<?php if ( $parallax ) { ?>
