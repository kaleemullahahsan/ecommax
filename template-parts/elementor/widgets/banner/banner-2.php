...
<div id="<?php echo esc_attr($id); ?>" class="rbb-banner group relative overflow-hidden rounded-[10px] <?php echo esc_attr($layout); ?>">
	<div class="relative">
		<div class="mx-auto absolute text-center px-10 z-10 top-1/2 -translate-y-1/2 transition-all lg:w-1/2 md:w-2/3 w-full right-0">
			<div class="block_heading">
				...
				<?php if ( $description ) { ?>
					<div class="desc text-white md:mb-8 mb-5"><?php echo wp_kses_post($description); ?></div>
				<?php } ?>
				<?php
				...
				foreach ( $buttons as $button ) {
					...
					<?php if ( $button_name ) { ?>
						<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-' . esc_attr($button['_id']) . '\', event)"' : ''; ?>
							...
							class="text-[#222222] hover:text-white inline-flex items-center justify-center transition-all <?php echo esc_attr($button_position); ?> <?php echo esc_attr($button_class); ?>"
							<?php
				...
				foreach ( $images as $image ) {
					...
					<?php if ( 0 === $img_count ) { ?>
						<div class="banner-img">
							...
							<?php } ?>
							<img alt="Image" src="<?php echo esc_url($image[ $widget->get_name_setting('image') ]['url']); ?>" class="md:min-h-full min-h-[400px] w-full scale-100 duration-300 group-hover:scale-105 <?php echo esc_attr(( $parallax ) ? 'layer' : ''); ?> inline-block <?php echo esc_attr($css_class); ?> <?php echo esc_attr($repeater_image_class); ?>"
							<?php if ( $parallax ) { ?>
