...
?>
<header id="rbb-default-header" class="rbb-default-header header-7 bg-[color:var(--rbb-header-background-color)]">
	<div class="relative z-30 w-full">
		<div class="md:block hidden 2xl:container px-[15px] mx-auto ">
			<div class="header-inner mx-auto h-full">
				<div class="mx-auto rbb-header-top flex items-center">
					...
					<div class="basis-4/5 flex w-full items-center justify-center">
						<?php if ( Helper::show_search_product_form() ) { ?>
							<div class="site-header-search mr-10 ml-5 xl:ml-0 grow">
							<?php if ( true === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_OVERLAY) ) { ?>
								<div class="rbb-product-search relative flex items-center justify-center cursor-pointer <?php echo $menu_vertical ? '' : 'rbb-product-search-zero'; ?>" onclick="RbbThemeSearch.openSearchForm(event)">
								...
								<?php } ?>
							<?php } else { ?>
								<div class="rbb-product-search relative flex items-center justify-center cursor-pointer w-full <?php echo $menu_vertical ? '' : 'rbb-product-search-zero'; ?>">
								...
								<?php if ( ! empty(Helper::get_popular_keyword()) ) { ?>
									<div class="host-search items-center justify-start mt-6 lg:flex hidden">
										<div class="heading-search text-xs font-bold uppercase text-white mr-2"><i class="rbb-icon-flash-1 mr-4"></i><?php echo esc_html__('SEARCH TRENDING :', 'ecommax'); ?></div>
										<ul class="rbb-search-popular overflow-x-auto flex text-white capitalize text-xs">
											<?php foreach ( Helper::get_popular_keyword() as $keyword ) { ?>
				...
				<div class="rbb-header-center gradient-animate <?php echo $menu_vertical ? '' : 'pt-2 pb-2'; ?>">
					<div class="mx-auto flex items-center justify-between">
						...
						<div class="basis-1/5 header-center-left min-w-[300px] all-categories pt-2 pb-2 rissing_overlay_blur" data-backdrop="<?php echo esc_attr($class_string); ?>" data-repond="1024">
							<div class="duration-300 cursor-pointer flex items-center text-[#181818] relative categories-text pl-5 pr-6">
								...
								<span class="text-[12px] uppercase font-bold text-[#181818]"><?php echo esc_html__('Browse All Category', 'ecommax'); ?></span>
						...
						<div class="header-center-right flex items-center justify-between <?php echo $menu_vertical ? 'basis-4/5' : 'basis-full w-full'; ?>">
							...
							<?php if ( is_active_sidebar('header-contact-right2') ) { ?>
								<div class="flex justify-end w-full">
									<?php dynamic_sidebar('header-contact-right2'); ?>
								</div>
							<?php } ?>
						</div>
					</div>
	...
	<div class="sticky_header">
		<div class="rbb-header-sticky relative z-50 opacity-0 invisible bg-[color:var(--rbb-menu-sticky-background-color)]">
