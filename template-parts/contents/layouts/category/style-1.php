	...
	<div class="blog-wrap">
		...
		<div class="-mt-[104px] relative w-[87.5%] bg-white rounded-tr-[28px]">
				...
				<div class="pt-7 flex font-semibold text-[#909090] text-[10px] uppercase">
					<?php if ( Setting::get(RISING_BAMBOO_KIRKI_FIELD_BLOG_CATEGORY_SHOW_AUTHOR) ) { ?>
						<span class="pr-7 flex items-center">
							...
							<?php } ?>
							<?php echo esc_html__('By ', 'ecommax'); ?><?php echo get_the_author(); ?>
						</span>
					<?php } ?>
						...
						<span class="comments-link inline-flex">
							...
							if ( $comment_count > 0 ) {
								...
								<?php if ( 1 === $comment_count ) { ?>
									<?php echo esc_attr($comment_count) . '<span>' . esc_html_e('Comment', 'ecommax') . '</span>'; ?>
								<?php } else { ?>
									<?php echo esc_attr($comment_count) . '<span>' . esc_html__(' Comments', 'ecommax') . '</span>'; ?>
								<?php } ?>
							<?php } else { ?>
								<?php echo esc_attr($comment_count) . '<span>' . esc_html__(' Comments', 'ecommax') . '</span>'; ?>
							<?php } ?>
			...
			<div class="entry-content mb-[30px]">
				...
				<?php } ?>
				<div class="my-5">
					<a href="<?php the_permalink(); ?>" class="blog-readmore flex items-center text-[#bebebe] hover:text-[color:var(--rbb-general-primary-color)] font-semibold relative mt-6 mb-4"><span class="mr-1.5 relative"><?php echo esc_html__('Read more', 'ecommax'); ?></span>
						<?php if ( class_exists(RbbIcons::class) ) { ?>
